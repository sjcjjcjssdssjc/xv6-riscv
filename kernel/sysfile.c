...
/*
  if((fp = nameiparent(path, name)) == 0)// a/b/c/d: name is d, dp is inode number of c(father of d)
    return -1;
  printf("%s %s\n",path,name);
  ilock(fp);
  printf("%d\n",dirlookup(fp, path, 0));//lock is not ok
  iunlockput(fp);
*/
